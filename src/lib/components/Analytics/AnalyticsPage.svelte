<script lang="ts">
	...
	onMount(async () => {
		...
		d3BarChart(
			...
			[
				...
				Math.max.apply(
					Math,
					stats.new_installs.map(({ count, date }) => count)
				)
			],
			newInstallsDiv.getBoundingClientRect().width,
			newInstallsDiv.getBoundingClientRect().height,
			'#77B924'
		);

		d3BarChartH(
			tutorialCompletionDiv,
			stats.tutorial_completion
				.map(({ count, key }) => {
			...
			[
				...
				Math.max.apply(
					Math,
					stats.tutorial_completion.map(({ count, key }) => count)
				)
			],
			tutorialCompletionDiv.getBoundingClientRect().width,
			tutorialCompletionDiv.getBoundingClientRect().height,
			'#64A6E2'
		);
	});
	...
	const d3BarChartH = (
		...
		const y = d3
			...
			.padding(0.2);
		svg.append('g').call(d3.axisLeft(y)).selectAll('text').style('text-anchor', 'end');
...
<section class="stats">
	<div class="card">
		...
		<span class="value">
			{#if stats} {stats.unique_installs} {/if}</span
		>
	</div>
	...
	<div class="card">
		...
		<span class="value">
			{#if stats} {stats.planted_trees} {/if}</span
		>
	</div>
	<div class="card">
		<span class="title"> Gespielte Quizze </span>
		<span class="value">
			{#if stats} {stats.played_quizzes} {/if}</span
		>
	</div>
	<div class="card">
		<span class="title"> Abgeschlossene Challenges </span>
		<span class="value">
			{#if stats} {stats.played_quests} {/if}</span
		>
	</div>
